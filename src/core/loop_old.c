...
void server_loop(Server *server, Uring *uring)
{
	...
	while (1)
	{
		...
		while (1)
		{
			...
			{
				case READ:
					res = handle_read_result(server, message, cqe->res);

					switch (res)
					{
						case RESPOND:
							res = prepare_write(ring, socket, message);
							if(res == QUEUE_FULL)
								goto submit;
							submissions++;
							break;
						case RETRY:
							res = prepare_read(ring, socket, message);
							if(res == QUEUE_FULL)
								goto submit;
							else if (res != 0)
								return res;
					...
					break;
				case WRITE:
					res = handle_write_result(server, message, cqe->res);
					...
					else return_buffer(server, message);
					break;
				default:
